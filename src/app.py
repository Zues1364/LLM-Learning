...
async def ask_question(request: QueryRequest):
    ...
    if not selected_files and _query_requires_transcript_files(query):
        return {
            "answer": (
    ...
    try:
        ...
        if STRUCTURED_TKB_ENABLED:
            route = _structured_intent_classifier(query)
        ...
        planner_orchestration_required = (
            ...
            and _query_requires_planner_orchestration(query=query, route_intent=route_intent)
        )
        ...
        obj: Dict[str, Any]
        advisor_priority_query = bool(selected_files) and _query_requires_advisor_priority(query)
        if (
            STRUCTURED_TKB_ENABLED
            ...
            selected_files
            and _query_requires_transcript_files(query)
        ...
        if source in {"vector_store", "vector_store_compare"} and selected_files and _query_prefers_global_resource_retrieval(query):
            try:
        ...
        if source == "academic_advisor":
            ...
            answer = _render_structured_schedule_answer(query=query, context=context)
        elif source == "time_slot_lookup":
            answer = _render_time_slot_lookup_answer(query=query, context=context)
